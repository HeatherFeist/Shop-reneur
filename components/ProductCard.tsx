...
import { ShoppingCart, Gift, Trash2, Video, X, Edit2, Share2, Facebook, Twitter, Link, ChevronLeft, ChevronRight, Package, ShieldCheck, ArrowRight, ShoppingBag, Lock, Play } from 'lucide-react';
...
const ProductCard: React.FC<ProductCardProps> = ({ product, userRole, onDelete, onEdit, onAddToCart }) => {
  ...
  // Enterprise Grade = Has Review AND Stock for customer (at least 1 after admin takes theirs)
  const isUnlocked = hasReview && stockCount > 0;
    ...
    <div className={`glass-card rounded-[2.5rem] overflow-hidden group border transition-all duration-500 flex flex-col h-full bg-[#ffffff]/[0.02] ${isUnlocked ? 'border-indigo-500/30 ring-1 ring-indigo-500/10' : 'border-white/5 hover:border-white/20'}`}>
      <div className="relative h-72 overflow-hidden bg-slate-900">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
           {!hasReview ? (
             <span className="bg-yellow-500/20 backdrop-blur-md text-yellow-300 border border-yellow-500/30 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5">
               <Video size={10} /> Pending Review
             </span>
           ) : stockCount === 0 ? (
             <span className="bg-orange-500/20 backdrop-blur-md text-orange-300 border border-orange-500/30 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5">
               <Package size={10} /> Out of Stock
             </span>
           ) : (
        ...
        {product.videoUrl && (
           <a href={product.videoUrl} target="_blank" className="absolute bottom-4 left-4 z-10 p-3 bg-white/10 backdrop-blur-lg rounded-full border border-white/20 text-white hover:bg-indigo-600 transition-all">
        ...
        <div className="pt-4 mt-auto space-y-3">
          <div className="flex gap-2">
            {isUnlocked ? (
              ...
              >
                <Lock size={12} className="mr-1" /> Sponsorship Required
              </button>
            )}
          </div>
          ...
          {!isUnlocked && (
            <div className="px-4 py-2 bg-white/5 rounded-xl border border-white/5">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">
                 {!hasReview ? "Step 1: Admin Video Review Required" : "Step 2: Purchase Customer Stock"}
              </p>
            </div>
          )}
          ...
          {userRole === 'admin' && (
             <div className="flex gap-2">
                ...
                <button className="flex-1 p-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white hover:bg-white/10 transition-all">Strategize</button>
             </div>
          )}
        </div>
      </div>
