...
import { ShoppingCart, Gift, Trash2, Video, X, Edit2, Share2, Facebook, Twitter, Link, ChevronLeft, ChevronRight, Package, ShieldCheck, ArrowRight } from 'lucide-react';
      ...
      <div className="relative h-72 overflow-hidden bg-slate-900">
        ...
        {allImages.length > 1 && (
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex((prev) => (prev - 1 + allImages.length) % allImages.length); }} className="p-2 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-white/20"><ChevronLeft size={16} /></button>
            <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex((prev) => (prev + 1) % allImages.length); }} className="p-2 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-white/20"><ChevronRight size={16} /></button>
          </div>
        )}
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
           {totalStock === 0 ? (
             <span className="bg-indigo-500/20 backdrop-blur-md text-indigo-300 border border-indigo-500/30 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">Wishlist Phase</span>
           ) : (
             <span className="bg-emerald-500/20 backdrop-blur-md text-emerald-300 border border-emerald-500/30 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5">
               <ShieldCheck size={10} /> Incubator Asset
             </span>
           )}
           {product.isMarketplaceSynced && (
             <span className="bg-amber-500/20 backdrop-blur-md text-amber-300 border border-amber-500/30 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">Platform Sync Verified</span>
           )}
        </div>
      </div>
        ...
        <div className="pt-4 mt-auto space-y-3">
          <div className="flex gap-2">
            {canBuyFromTeen ? (
              ...
              >
                Acquire Now <ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
              </button>
            ) : (
              <button 
          ...
          {userRole === 'admin' && (
             <div className="flex gap-2">
                ...
                <button className="flex-1 p-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white hover:bg-white/10 transition-all">Edit Metadata</button>
             </div>
          )}
        </div>
      </div>
