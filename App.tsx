...
const App: React.FC = () => {
  ...
  const handleUploadReview = (productId: string, videoUrl: string) => {
    const product = products.find(p => p.id === productId);
    if (product) {
      handleUpdateProduct({ ...product, videoUrl, videoReviewCompleted: true });
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#020617] text-slate-200 selection:bg-indigo-500/30">
      ...
      <CartDrawer isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} cartItems={cart} onRemoveItem={(id) => setCart(cart.filter(i => i.id !== id))} shopSettings={shopSettings} userProfile={currentUser || undefined} onPurchaseComplete={handlePurchaseComplete} onUploadReview={handleUploadReview} />
